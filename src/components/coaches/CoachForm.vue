<template>
  <form @submit.prevent="submitForm">
   ...
   <p v-if="!firstName.isValid" > First Name must not be empty </p>
     ...
     <div class="form-control" :class="{invalid:!descripton.isValid}">
      <label for="description">description</label>
      <textarea rows="5" id="description" v-model.trim="descripton.val" @blur="clearValidity('descripton')"></textarea>
        <p v-if="!descripton.isValid" >description must not be empty </p>
    </div>
     <div class="form-control" :class="{invalid:!rate.isValid}">
      <label for="rate">hourlyRate</label>
      <input type="number" id="rate" v-model.number="rate.val" @blur="clearValidity('rate')" />
        <p v-if="!rate.isValid" >rate  must be grater than Zero </p>
    </div>
     <div class="form-control" :class="{invalid:!areas.isValid}">
      ...
      <div>
          <input type="checkbox" id="career" value="career" v-model="areas.val" @blur="clearValidity('areas')"/>
           ...
           <p v-if="!areas.isValid" >must select one experience at least </p>
    </div>
    <p v-if="!formIsVslid">please fix the above errors and submit again </p>
      ...
      clearValidity(input)
      {
        ...
        validateForm()
        {
            ...
            } if (this.lastName.val==='')
            {
             ...
             if (this.descripton.val==='')
            {
              this.descripton.isValid=false;
              this.formIsValid=false;
            }
             if (!this.rate.val || this.ra.val<0)
            {
        ...
        submitForm()
        {
            ...
            if(!this.formIsValid)
          {
            console.log(this.areas.isValid);
            return ;
          }
            const formData= {
                first:this.firstName.val,
               last: this.lastName.val,
               desc:this.descripton.val,
               rate:this.rate.val,
